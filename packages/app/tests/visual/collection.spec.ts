...
test('collection route renders the mint gallery shell', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: /View contract/i })).toBeVisible()
...
test('mobile collection route keeps controls visible', async ({ page }) => {
  ...
  const mintFullSet = page.getByRole('button', { name: /Mint full set/i })
  await expect(mintFullSet).toBeVisible()
  await expect(mintFullSet).toBeEnabled()
  await expect(page.getByRole('button', { name: /Refresh/i })).toHaveCount(0)
})

test('disconnected mint action opens the connect flow', async ({ page }) => {
  await page.goto('/collections/drums-collection-1')

  await page.getByRole('button', { name: /Mint full set/i }).click()
